...
final class APIClient {
    ...
    func deleteAvatar() async throws -> AvatarUploadResponse {
        try await request("api/upload/avatar", method: "DELETE")
    }

    func uploadGroupAvatar(conversationId: String, data: Data, filename: String, mimeType: String) async throws -> DirectConversationResponse {
        var request = URLRequest(url: APIClient.baseURL.appendingPathComponent("api/conversations/\(conversationId)/avatar"))
        request.httpMethod = "POST"
        let boundary = "Boundary-\(UUID().uuidString)"
        request.setValue("multipart/form-data; boundary=\(boundary)", forHTTPHeaderField: "Content-Type")
        if let token {
        ...
        if !(200...299).contains(http.statusCode) {
            ...
            throw APIError.server("Не удалось загрузить фото группы (\(http.statusCode))")
        }
        return try decoder.decode(DirectConversationResponse.self, from: responseData)
    }

    func deleteGroupAvatar(conversationId: String) async throws -> DirectConversationResponse {
        try await request("api/conversations/\(conversationId)/avatar", method: "DELETE")
    }
